...
var calc = function(){
    ...
    if(arg[arg.length-1] === 'e'){
        ...
        if(Sqrt(Pow(arg[5]-x_i,2)+Pow(arg[6]-y_i,2))<Sqrt(Pow(arg[5]-x_ii,2)+Pow(arg[6]-y_ii,2))){
            ret[0] = Round(x_i); // use pythagoras to determin point on circumference
            ret[1] = Round(y_i); // which is closest to the ball (as this is where it will collide)
        }               
...
var test = function(x,y,a,cx,v,next){ // check the parameters
  ...
  var CP = calc(mycirc.cpx,mycirc.cpy,myball.g,myball.i,mycirc.rad,myball.x_pos,myball.y_pos,'c');
  var RL;
  ...
  if(x === 500){
    ...
    if(v === ['+','+']){
    ...
    else if(typeof CP !== 'undefined'){ // this can stay the same 
    
      var CL = calc(mycirc.cpx,mycirc.cpy,CP[0],CP[1],'e'); 
      var PL = calc(CP[0],CP[1],CL[0],'p');   
      var para_i = myball.y_pos - CL[0]*myball.x_pos;
      var PV = [CL[0],para_i];    
      var p_x = (PL[1]-PV[1])/(CL[0]-PL[0]); 
      var p_y = p_x*CL[0] + PV[1];    
      var len_adj = Sqrt(Pow(p_x-CP[0],2)+Pow(p_y-CP[1],2));
      var len_opp = Sqrt(Pow(p_x-myball.x_pos,2)+Pow(p_y-myball.y_pos,2));    
      var refl_angle = atan(len_opp/len_adj);
      var refl_grad;    
      var DG = calc(myball.x_pos,myball.y_pos,mycirc.cpx,mycirc.cpy,'e'); 
      ...
      RL = [refl_grad,refl_i,refl_angle];   
      draw_line(0,RL[1],500,RL[1]+500*RL[0]);
  }
   return [CP,RL[0],next]; // check what is being returned

};

var com = function(bx,by,ba,cx,CP,RG,next,v){
  console.log(ba);
  ...
  if(next === true){
    ...
    else if(ba > 90 && ba < 180){
      v = ['-','+'];
    }
    else if(ba > 180 && ba < 270){
      v = ['-','-'];
    }
    else if(ba >= 270 && ba < 360){
      v = ['+','-'];
    }
  }
  ...
  var a = test(bx,by,ba,cx,next);  // check what is being entered
  
  return[bx,by,ba,a[2],v];  // check what is being returned
};
  


var vars = [0,250,315,false,['+','-']];
var cx = 250;
var v = test(vars[0],vars[1],vars[2],cx,vars[4],vars[3]);
var si;


$(document).ready(function(){
  si = setInterval(function(){vars = com(vars[0],vars[1],vars[2],cx,v[0],v[1],vars[3],vars[4]);},10); // bx,by,ba,circlex,centre point,refl grad,next,vector
